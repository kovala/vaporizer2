...
enum EditorFunction {
  Level, Phase, Freq, FreqMod, BitDepth, Offset, Bend, Bloat, Smooth, Clean, Connect, DrawConnect, Normalize, NormalizePerCycle, AmplModCopy, MixInCopy, Interpolate, PWMGenerate, FMGenerate, HardSyncGenerate, BloatGenerate, BendGenerate, FormantsMaleGenerate, FormantsFemaleGenerate, HarmonicMorph, HarmonicReduce, HighpassFilter, LowpassFilter, LowpassSoft, BandpassFilter, PulseFilter, CombFilter, Sine, Zeros, Sawtooth, Sinc, HalfSine, Square, Triangle, Ramp, Random, Invert, Reverse, ArcConnect, RampConnect, SampleHold, Convolution, TimeToFrequency, FrequencyToTime,
};
...
public:
  ...
  void setCurWavetable(const std::shared_ptr<CVASTWaveTable>& wavetable) {
    //const is faster https://stackoverflow.com/questions/3310737/should-we-pass-a-shared-ptr-by-reference-or-by-value#:~:text=Most%20of%20the%20time%20passing,by%20const%20reference%2C%20would%20do.&text=An%20example%20of%20when%20passing,the%20callee%20completes%20its%20job.
  ...
  void setCopyPasteWavetable(const std::shared_ptr<CVASTWaveTable>& wavetable) {
    //const is faster https://stackoverflow.com/questions/3310737/should-we-pass-a-shared-ptr-by-reference-or-by-value#:~:text=Most%20of%20the%20time%20passing,by%20const%20reference%2C%20would%20do.&text=An%20example%20of%20when%20passing,the%20callee%20completes%20its%20job.
  ...
  void setWtPos(int wtpos) {
    //normally use setSelectedWtPos on Wavetable!!
